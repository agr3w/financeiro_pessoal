...
import { CheckCircle, Schedule, DeleteOutline, EventRepeat, Autorenew } from '@mui/icons-material';
...
export default function LoanTracker({ loan, onPay }) {
  ...
  return (
    <Card 
        elevation={0}
        sx={{ 
            mb: 2, borderRadius: 3, position: 'relative',
    ...
    >
      ...
      <CardContent>
        ...
        <Box 
            sx={{ 
                p: 2, 
                bgcolor: isSubscription ? alpha(theme.palette.info.main, 0.05) : alpha(theme.palette.secondary.main, 0.05), // Cor diferente pra assinatura
                borderRadius: 2, 
        ...
        >
          <Box>
            ...
            <Typography variant="h6" fontWeight="bold" color={isSubscription ? "info.main" : "secondary.main"}>
                R$ {currentInstallment.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </Typography>
            <Typography variant="caption" color="text.secondary">
                {isSubscription ? 'Renovação Mensal' : `Parcela ${currentInstallment.number} / ${totalInstallments}`}
            </Typography>
          </Box>
