...
const PAYMENT_METHODS = [
  ...
  { id: 'voucher', label: 'Vale (VR/VA)', icon: <LocalDining fontSize="small" /> },
];
  ...
  return (
    <>
      ...
      <Dialog 
        ...
        fullWidth 
        maxWidth="xs"
        PaperProps={{ sx: { borderRadius: fullScreen ? 0 : 5, bgcolor: theme.palette.background.paper } }}
      >
        <Box sx={{ position: 'relative', p: 1 }}>
          ...
          <DialogContent sx={{ pt: 4, pb: 3, display: 'flex', flexDirection: 'column', gap: 3 }}>
                ...
                {displayCategories.map((cat) => {
                  ...
                  return (
                    <Grid item xs={3} key={cat.id || cat.label} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                      ...
                      <Typography variant="caption" noWrap sx={{ mt: 1, fontWeight: isSelected ? 700 : 500, color: isSelected ? catColor : 'text.secondary', maxWidth: '100%' }}>{cat.label}</Typography>
                    </Grid>
                  )
                })}
              </Grid>
            ...
            {type === 'expense' && (
                <Box>
                    <Typography variant="caption" align="center" display="block" color="text.secondary" mb={1.5} fontWeight={600} letterSpacing={1}>
                        PAGAR COM
                    </Typography>
            ...
            <Box sx={{ mt: 1 }}>
                <TextField
                    placeholder="Descrição opcional" variant="filled" size="small"
