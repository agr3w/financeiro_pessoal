...
import {
  ...
  Dialog, TextField, DialogTitle, DialogContent, DialogActions, Alert, InputAdornment
} from '@mui/material';
import {
  DarkMode, LockReset, DeleteForever, Security, Image as ImageIcon, Link as LinkIcon, Close
...
const WALLPAPER_PRESETS = [
  { id: 'mountains', url: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?q=80&w=2000&auto=format&fit=crop', label: 'Montanhas' },
  { id: 'abstract', url: 'https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=2000&auto=format&fit=crop', label: 'Abstrato' },
  { id: 'lofi', url: 'https://images.unsplash.com/photo-1516280440614-6697288d5d38?q=80&w=2000&auto=format&fit=crop', label: 'Noite' },
  { id: 'plants', url: 'https://images.unsplash.com/photo-1470058869958-2a77ade41c02?q=80&w=2000&auto=format&fit=crop', label: 'Natureza' },
  { id: 'clouds', url: 'https://images.unsplash.com/photo-1534088568595-a066f410bcda?q=80&w=2000&auto=format&fit=crop', label: 'Nuvens' },
];
...
export default function Settings() {
  const { mode, toggleColorMode, bgImage, setCustomBackground } = useThemeContext();
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              <Typography variant="h6" fontFamily="serif" mb={2}>Visual</Typography>
              ...
              <Box>
                <Box display="flex" gap={2} alignItems="center" mb={2}>
                  <Box sx={{ p: 1, bgcolor: 'action.hover', borderRadius: 2, color: 'text.secondary' }}>
                    <ImageIcon />
                  </Box>
                  <Box>
                    <Typography variant="subtitle1" fontWeight="bold">Papel de Parede</Typography>
                    <Typography variant="caption" color="text.secondary">Personalize o fundo do seu dashboard</Typography>
                  </Box>
                </Box>
                ...
                <Box display="flex" gap={1} mb={2}>
                  ...
                  <Button
                    variant="contained"
                    disabled={!customUrl}
                    onClick={() => { setCustomBackground(customUrl); setCustomUrl(''); }}
                  >
                    Aplicar
                  </Button>
                </Box>
                ...
                {/* Grid de Sugestões */}
                <Typography variant="caption" fontWeight="bold" color="text.secondary" mb={1} display="block">SUGESTÕES</Typography>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              <Typography variant="h6" fontFamily="serif" mb={2}>Conta</Typography>
              <SettingItem
                icon={<LockReset />} title="Alterar Senha" description="Atualize sua senha de acesso"
                action={<Button variant="outlined" size="small" onClick={() => setOpenPassModal(true)}>Alterar</Button>}
              />
              <Divider sx={{ my: 1 }} />
              <SettingItem
                icon={<Security />} title="E-mail" description={useAuth().user?.email}
                action={<Button size="small" disabled>Fixo</Button>}
              />
            </Paper>
          </Grid>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 3, borderRadius: 3, border: '1px solid #ffcdd2' }}>
              ...
              <SettingItem
                danger icon={<DeleteForever />} title="Excluir Conta" description="Apagar todos os dados permanentemente"
                action={<Button variant="contained" color="error" size="small" onClick={() => setOpenDeleteModal(true)}>Excluir</Button>}
              />
            </Paper>
          </Grid>
