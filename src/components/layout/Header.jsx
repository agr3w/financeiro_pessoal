      ...
      <Box width="100%">
        ...
        <Box>
          ...
          >
            {expanded
              ? note.message
              : `${note.message.substring(0, LIMIT)}${isLong ? "..." : ""}`}
          </Typography>
          ...
          {isLong && (
            <Button
              ...
              endIcon={
                expanded ? (
                  <ExpandLess sx={{ width: 14 }} />
                ) : (
                  <ExpandMore sx={{ width: 14 }} />
                )
              }
            >
  ...
  useEffect(() => {
    if (notifications && notifications.length > 0) {
      const lastSeenCount = parseInt(
        localStorage.getItem("lastSeenNotifCount") || "0",
      );
      ...
      <Container maxWidth="lg">
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box>
            {title ? (
              <>
                <Typography variant="h5" fontFamily="serif" fontWeight="bold">
                  {title}
                </Typography>
                {subtitle && (
