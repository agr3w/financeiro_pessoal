...
export const CustomThemeProvider = ({ children }) => {
  ...
  const getDesignTokens = (mode) => ({
    palette: {
      ...
      background: { 
        ...
        default: bgImage 
            ? (mode === 'light' ? 'rgba(244, 246, 248, 0.85)' : 'rgba(15, 18, 20, 0.9)') 
        ...
        styleOverrides: {
          root: {
            ...
            backgroundColor: bgImage 
                ? (mode === 'light' ? 'rgba(255, 255, 255, 0.8)' : 'rgba(26, 29, 31, 0.8)') 
                : undefined,
            boxShadow: mode === 'light' ? '0px 2px 4px rgba(0,0,0,0.02), 0px 8px 24px rgba(0,0,0,0.04)' : '0px 2px 4px rgba(0,0,0,0.2)',
        ...
        styleOverrides: {
          root: {
            '& .MuiOutlinedInput-root': {
              borderRadius: 12,
              backgroundColor: mode === 'light' ? 'rgba(255,255,255,0.8)' : 'rgba(37, 40, 44, 0.8)', // Levemente transparente
  ...
  const theme = useMemo(() => createTheme(getDesignTokens(mode)), [mode, bgImage]); // Recria o tema se a imagem mudar

  return (
    <ThemeContext.Provider value={{ 
        mode, toggleColorMode, 
        dashboardPrefs, toggleDashboardPref,
        bgImage, setCustomBackground // Exporta as novas funções
    }}>
